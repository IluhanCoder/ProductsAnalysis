...
export default new class TransactionService {
    ...
    async fetchTransactions(filter: {date: {gte: Date, lte: Date}}, productName: string) {
        ...
        filter.date.gte.setUTCHours(startDate.getHours(), startDate.getMinutes(), startDate.getSeconds());
        filter.date.lte.setUTCHours(endDate.getHours(), endDate.getMinutes(), endDate.getSeconds());
