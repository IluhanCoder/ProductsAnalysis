import { AnalyticsResult, AverageTransactions, GraphFormat, GraphUnit, MonthlyTransactionAmount, MonthlyTransactionSum } from "./analytics-types";

export function convertAnalyticsForGraphs(analytics: AnalyticsResult): GraphFormat {
    ...
    analytics.monthlyTransactionSum.map((unit: MonthlyTransactionSum) => {
        monthlyTransactionSum.push({name: unit.month, uv: unit.transactionCostsSum})
    })
    return {
        monthlyTransactionAmount: {name: "Кількість транзакцій",
        data: monthlyTransactionAmount},
        averageTransactions: {name: "Середня сума чеку (грн)",
        data: monthlyTransactionSum},
        monthlyTransactionSum: {name: "Сума продажів (грн)",
