...
export default new class AnalyticsService {
    ...
    async getAnalyticsData (startDate: Date, endDate: Date, ) {
        ...
        const averageTransactions = (await $api.post("/average-transaction", {startMonth, endMonth})).data;
        const monthlyTransactionSum = (await $api.post("/transaction-sum", {startMonth, endMonth})).data;
